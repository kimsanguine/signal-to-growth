...
- **[2026-08-04] Round 3 자동화 — 정책 강제를 Naver·Channel Talk까지 확장.** Round 2에서 범위 밖이던 두 어댑터의 `allow_unverified_fixture` 플래그를 제거하고 Kakao와 동일하게 `policy.py` 공용 로더 + `blocked_verification_assurance` 대조로 교체. Channel Talk은 webhook 경로뿐 아니라 **backfill 경로도** 강제 대상에 포함 — 미인증 transport가 webhook이 거부했을 이벤트를 대신 주입하는 구멍을 막음. 호출부 3곳(`cli.py`의 naver·channel-talk, fixture 테스트)은 `fixture_ingest_policy()` 명시. 가역성: 2-way door. 검증: 정책 파일만 완화한 임시 복사본(`SIGNAL_TO_GROWTH_ROOT`)에서 두 어댑터가 `none`을 수용하도록 바뀌는 것을 직접 관측 + `tests/test_policy.py` 신규 8케이스.
- **[2026-08-04] Round 3 자동화 — `connectors.default_mode`는 "예약 필드"로 정직하게 표기.** 강제 지점을 새로 만들지 않음. 이유(실측): 이 릴리스에 outbound write 경로 자체가 없음 — `BaseChannelAdapter.send_approved`가 무조건 `UnsupportedCapability`를 던지고 세 어댑터 모두 `REPLY_SEND`를 `disabled_by_policy`에 둠. 즉 mode가 게이트할 대상이 존재하지 않음. 있는 척하는 대신 정책 파일(`default_mode_note`)과 로더(`ConnectorPolicy.default_mode` 주석) 양쪽에 미강제 사실과 향후 연결 지점을 명시.
- **[2026-08-04] Round 3 자동화 — 성공 경로 로그 추가.** `StructuredLogger.ingest_accepted`(기존 `emit()` allowlist 재사용, `duration_ms` 필드 추가)를 `kakao_skill_server.py`의 sink 성공 직후 1줄 호출. 이전에는 실패 4곳만 로깅해 "정상 운영"과 "웹훅 끊김"이 둘 다 침묵으로 동일했음. 원문 발화·시크릿은 기존 redact 계약 그대로 차단(테스트로 고정).
- **[2026-08-04] Round 3 자동화 — 503 재시도 docstring 완화.** `kakao_skill_server.py` 모듈 docstring의 "Nothing is lost" 단정문을 "설계 의도이며 provider 실제 재시도 동작은 미관측"으로 조건부 표기. **코드 로직 무변경** — 표현만 실제 검증 수준에 맞춤(열린 이슈 2번과 일치).
