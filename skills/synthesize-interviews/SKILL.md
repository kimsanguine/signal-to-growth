...
Add every new record to `evidence.jsonl` with the `append-record` command,
never by writing or editing the file:

```bash
python3 scripts/stg.py append-record <artifact-directory>/evidence.jsonl '<json-object>'
```
